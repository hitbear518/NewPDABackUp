...
JNINativeMethod g_queryMethodTable[] =
{
	...
	{"preGetInterfaceWarehouses", "()[B", (void *) preGetInterfaceWarehouses},
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
	...
	CHECK_NULL(loginCls);
	jint returnValue = env->RegisterNatives(loginCls, g_loginMethodTable, 11);
	if (returnValue < 0)
	{
	...
	CHECK_NULL(queryCls);
	returnValue = env->RegisterNatives(queryCls, g_queryMethodTable, 41);
	if (returnValue < 0)
	{
		env->Throw(env->ExceptionOccurred());
	}
