...
jbyteArray preGetSpecs(JNIEnv *env, jobject thiz, jint warehouseId, jstring barcode)
{
	...
	printf("%s", sql);
	const char* test_sql = "SELECT SpecID FROM g_goods_goodsspec LIMIT 5";
	status = tdi_prot_st_execute(g_tdi, sql);
	printf("preGetSpecs status=%d", status);
	CHECK_STATUS(status);
	if (status)
	{
		throwEx(env, status, tdi_prot_data(g_tdi));
...
jbyteArray preGetInterfaceWarehouses(JNIEnv *env, jobject thiz)
{
	int status;
	...
	const char *where = "WHERE w.bBlockUp=0 AND w.WarehouseType<1 AND iw.InterfaceID='pda' and iw.WarehouseNO<>'' ORDER BY w.WarehouseID ASC";

	char sql[strlen(select) + strlen(from) + strlen(left_join) + strlen(where) + 3];
	sprintf(sql, "%s %s %s %s", select, from, left_join, where);
	printf("%s", sql);
	status = tdi_prot_st_execute(g_tdi, sql);
	if (status)
	{
...
int getSqlResult(JNIEnv *env, jbyteArray jbytes)
{
	...
	printf("************************** getCustomers, status = %d", status);
	CHECK_STATUS(status);
	return status;
}
