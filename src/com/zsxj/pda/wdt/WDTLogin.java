	...
	public class LoginRunnable implements Runnable {
		...
		public void run() {
			...
			try {						
				...
				byte[] bytes5 = prepareGetUserId(mUserName);
				if (null == bytes5) {
					mCallBack.onLoginFail(HandlerCases.PREPARE_GET_USER_ID_FAIL, null);
					return;
				}
				ops.write(bytes5);
				recvLen = ips.read(recvBuf);
				int userId = getUserId(recvBuf);
				if (userId == -1) {
					mCallBack.onLoginFail(HandlerCases.GET_USER_ID_FAIL, null);
					return;
				}
				...
				mCallBack.onLoginSuccess(userId, accounts);
